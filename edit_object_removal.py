# This file is part of TF-OVOR: TF-OVOR: Efficient Object-Aware 3D Inpainting via Gaussian Splatting for 360Â° Scenes
...
def visualize_and_save_ply(points, labels, ply_path, mask=None):
    ...
    print(f"âœ?color point cloud saved to {ply_path}")
...
def visualize_rgb_and_save_ply(points, features_dc, ply_path, mask=None):
    ...
    print(f"ðŸŽ¨ RGB via SH DC: {valid_points.shape[0]} points â†?{ply_path}")
...
def removal_setup(model_path, iteration, gaussians, classifier, selected_obj_ids, removal_thresh):
    ...
    if visualize_ply == True:
        ...
        # ðŸŽ¨ visualize `prob_obj3d` in color **
...
def combine_gaussian(dataset, iteration, gaussians, objects_model_dict, target_id):
    ...
    print(f"âœ?Success: Combined scene saved at {combined_gaussian_path}")
