# This file is part of TF-OVOR: TF-OVOR: Efficient Object-Aware 3D Inpainting via Gaussian Splatting for 360° Scenes
# Project page: https://dfki-av.github.io/TF-OVOR/
#
...
def fusion(dataset_path, model_path, name, iteration, views):
    ...
    depth_completed_path=os.path.join(model_path, name, "ours_object_removal/iteration_{}".format(iteration), "depth_completed")     # result from lama depth completion
    ...
    for idx, view in enumerate(tqdm(virtual_poses_list, desc="Color-Depth-Fusion progress")):
        ...
        possible_paths = [os.path.join(dataset_path, "images_inpaint_unseen_virtual", view.image_name + ext) for ext in extensions]
        ...
        mask = cv2.imread(os.path.join(dataset_path, "inpaint_2d_unseen_mask_virtual", view.image_name+".png"), cv2.IMREAD_GRAYSCALE).astype(bool).reshape(-1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", type=str, default="config/object_removal/TF-OVOR/picnic.json", help="Path to the configuration file")
