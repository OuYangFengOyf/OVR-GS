...
def inpaint(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, opt : OptimizationParams, select_obj_id : int, removal_thresh : float,  finetune_iteration: int, render_video : bool, args, config):
    ...
    with torch.no_grad():
        ...
        if "TF-OVOR" in args.source_path:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", type=str, default="config/object_inpaint/TF-OVOR/doppelherz.json", help="Path to the configuration file")
    args = get_combined_args(parser)
    
    # a better initialization could be selected through 'images_inpaint_unseen_virtual' manually
    args.supp_ply = os.path.join(args.model_path, 'virtual/ours_object_removal/iteration_2000/fused_mask_col_dep_ply/00004.ply')  
