...
def render_set_removal_stage(model_path, name, iteration, views, gaussians, pipeline, background, classifier):
    """
...
def  virtual(dataset : ModelParams, iteration : int, pipeline : PipelineParams):
    with torch.no_grad():
        ...
        assets_dir = os.path.join(os.path.dirname(__file__), "..", "Segment-and-Track-Anything", "assets")
        zip_path = os.path.join(assets_dir, "images")
        print(f"\n📦 Packaging render results: {renders_dir} -> {zip_path}.zip")
        shutil.make_archive(zip_path, 'zip', renders_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", type=str, default="config/object_removal/TF-OVOR/doppelherz.json", help="Path to the configuration file")
